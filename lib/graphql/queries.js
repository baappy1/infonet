...
// Full Yoast SEO fields (add-wpgraphql-seo / Yoast)
...
export const GET_PAGE_SEO_BY_ID = gql`
  query GetPageSeoById($pageId: Int!) {
    pageBy(pageId: $pageId) {
      id
      databaseId
      title
...
/** Default site SEO (Yoast on the front page WP Page). Use pageBy — `post` is for blog posts, not Pages. */
export const GET_DEFAULT_SEO = gql`
  query DefaultSEO($pageId: Int!) {
    pageBy(pageId: $pageId) {
      id
      databaseId
      title
...
export const GET_POST_SEO_BY_ID = gql`
  query GetPostSeoById($postId: Int!) {
...
export const GET_SOLUTION_SEO_BY_ID = gql`
  query GetSolutionSeoById($solutionId: Int!) {
    solutionBy(solutionId: $solutionId) {
      id
      databaseId
      title
      excerpt
...
export const GET_SERVICE_SEO_BY_ID = gql`
  query GetServiceSeoById($serviceId: Int!) {
    serviceBy(serviceId: $serviceId) {
      id
      databaseId
      title
      excerpt
...
export const GET_INDUSTRIE_SEO_BY_ID = gql`
  query GetIndustrieSeoById($industrieId: Int!) {
...
export function buildIndustriesQuery(ids) {
  ...
  const selections = ids
    .map((_, i) => `industry_${i}: industrieBy(industrieId: $id${i}) { ${fragment} }`)
    .join("\n");
...
// Service blocks by slug (for /service/[slug] dynamic page)
export const GET_SERVICE_BLOCKS_BY_SLUG = gql`
  query GetServiceBlocksBySlug($slug: String!) {
    services(first: 1, where: { name: $slug }) {
      nodes {
        id
        databaseId
...
// Service with blocks connection (for blocks with attributesJSON format)
// Uses serviceId - fetch service by slug first to get databaseId
export const GET_SERVICE_WITH_BLOCKS = gql`
  query GetServiceWithBlocks($serviceId: Int!) {
    serviceBy(serviceId: $serviceId) {
      id
      databaseId
      blocks {
...
// Categories for blog/insights filtering
