 ...
 * Prefers Yoast fields on the front page (DefaultSEO / pageBy); falls back to root seo query.
 * Cached per request so root layout + app/page can share one GraphQL round-trip.
 */
export const getHomepageSeoMetadata = cache(async () => {
  const pageId = resolveHomePageDatabaseId();

  try {
    const data = await fetchGraphQL(print(GET_DEFAULT_SEO), {
