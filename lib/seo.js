...
export function getPostSeoMetadata(post) {
  ...
  const seo = post.seo;
  const title = seo?.title || seo?.opengraphTitle || post.title || DEFAULT_TITLE;
  const description = seo?.metaDesc || seo?.opengraphDescription || (post.excerpt ? post.excerpt.replace(/<[^>]+>/g, '').trim() : '') || DEFAULT_DESCRIPTION;
  const image = seo?.opengraphImage?.sourceUrl || post.featuredImage?.node?.sourceUrl || post.featuredImage?.node?.mediaItemUrl || DEFAULT_OG_IMAGE;
  return getDefaultMetadata({
    title: title.trim(),
