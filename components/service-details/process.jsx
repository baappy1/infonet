...
const ProcessSection = () => {
    ...
    useEffect(() => {
        const stepsContainer = sectionRef.current?.querySelector('.steps-container');
        if (!stepsContainer) return;

        const stepEls = Array.from(stepsContainer.querySelectorAll('[id^="step-"]'));
        const dotEls = stepEls.map((el) => el.querySelector('span')).filter(Boolean);
        const cardEls = Array.from(sectionRef.current?.querySelectorAll('[data-card]') ?? []);
        ...
        const ctx = gsap.context(() => {
            ...
            // Initialize border
            if (borderFillRef.current) gsap.set(borderFillRef.current, { height: '0%' });
            ...
            const setActiveStep = (nextStep) => {
                ...
                const currentDot = stepsContainer.querySelector(`#step-${nextStep} span`);
                if (currentDot) {
                    gsap.to(currentDot, {
            ...
            ScrollTrigger.create({
                ...
                onUpdate: (self) => {
                    ...
                    if (borderRef.current && p > 0) {
                        ...
                        borderRef.current.style.borderLeftStyle = p > 0.5 ? 'solid' : 'dashed';
                    }
        ...
        <div className='pt-20 lg:pt-55 pb-20 lg:pb-55'>
            <div className='container lg:pr-0 lg:pl-0 pr-5 pl-5'>
                <div className="w-full lg:w-[40.7%] sm:mb-0 mb-20">
                    ...
                    <h2 className="heading-h2 mb-5">How We Guide You From Start to Finish</h2>
                    <p className="paragraph-text mb-5">We follow a proven, transparent process that keeps your project moving efficiently and ensures every detail is handled with care.</p>
                </div>

                <div className="flex md:flex-row flex-col mt-20 flex-wrap gap-[6%] justify-between" ref={sectionRef}>
